...
def agregar(inicio, dato):
	...
	if (inicio == None):
		...
		if( dato < datoini):
			...
			reco = inicio
			while (reco != None):
				posterior = reco.enlace #Estas dos lienas es lo mismo que:
				# datoreco = reco.dato # LCelda (reco, datoreco, posterior)
				# a datoreco no lo necesitamos aca
				# posterior es lo mismo que decir enlacereco

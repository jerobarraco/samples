...
class ZODBFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.mnuFile.AppendItem(self.mnuOpen)
        self.mnuOpenZEO = wx.MenuItem(self.mnuFile, wx.ID_OK,
					"&Open ZEO Instance...\tCtrl-U", "", wx.ITEM_NORMAL)
    ...
    def _set_child_icons(self, c, d):
        ...
        if isinstance(d, (dict, UserDict.UserDict, OOBTree)):
            ...
            self.db_layout_tree.SetItemImage(c, self.file_idx,
                                                 wx.TreeItemIcon_Normal)
    def createTree(self, filename='', zeo=False):
        """Create a new tree structure for when we open a file"""
        self.doClose()
        if zeo:
            self.db = open_zodb(filename, zeo=True)
            self.root = self.db_layout_tree.AddRoot(filename)
        else:
    ...
    def doOpenUrl(self, *event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            self.createTree(zeo=True, filename=str(dlg.GetValue()))
        dlg.Destroy()
